...
const DateTimeRangeItem = (props) => {
    ...
    return <>
        <Typography variant="h5" component="div" sx={{flexGrow: 1}}>By {name.toLowerCase()} date:</Typography>
        <Typography variant="h7" component="div" sx={{flexGrow: 1}}>{name} from:</Typography>
        <LocalizationProvider dateAdapter={AdapterDayjs}>
            <DateTimePicker onChange={value => setStartsAtFrom(value)}/>
        </LocalizationProvider>
        <Typography variant="h7" component="div" sx={{flexGrow: 1}}>{name} to:</Typography>
        <LocalizationProvider dateAdapter={AdapterDayjs}>
